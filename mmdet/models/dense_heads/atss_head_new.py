...
class ATSSHeadnew(AnchorHead):
    ...
    def __init__(self,
        ...
        super(ATSSHeadnew, self).__init__(num_classes, in_channels, **kwargs)
    ...
    def loss(self,
        ...
        if cls_reg_targets is None:
            return None
        # todo 获取targets, label ,label_weights, bbox_targets， bbox_weights
        (anchor_list, labels_list, label_weights_list, bbox_targets_list,
         bbox_weights_list, num_total_pos, num_total_neg) = cls_reg_targets
        # todo s0=16384,s1=4096,s2=1024,s3=256,s4=64
        # todo anchors_list=[5,[8,s,4]]
        ...
        # 原始的atss的权重设计，其中回归框的权重是由centerness采用
    ...
    def _get_bboxes_single(self,
        ...
        for cls_score, bbox_pred, centerness, anchors in zip(
            ...
            scores = cls_score.permute(1, 2, 0).reshape(
                -1, self.cls_out_channels).sigmoid()  # [h*w, 15]
            bbox_pred = bbox_pred.permute(1, 2, 0).reshape(-1, 4)  # [h*w, 4]
            centerness = centerness.permute(1, 2, 0).reshape(-1).sigmoid()  # [h*w, 1]

            # print(scores.shape)

            nms_pre = cfg.get('nms_pre', -1)
            if nms_pre > 0 and scores.shape[0] > nms_pre:
                # 根据分类的分数以及centeness得到最终的分数
                max_scores, _ = (scores * centerness[:, None]).max(dim=1)
                # 根据分数挑选出最大的前2000个
                _, topk_inds = max_scores.topk(nms_pre)
                # 获取anchors,以及bbox_pred,score, centerness
        ...
        mlvl_bboxes = torch.cat(mlvl_bboxes)  # [topk, 4]
        if rescale:
            mlvl_bboxes /= mlvl_bboxes.new_tensor(scale_factor)

        mlvl_scores = torch.cat(mlvl_scores)  # [topk, 15]
    ...
    def get_targets(self,
        ...
        # print("***")
        # todo anchor_list=[8, [21824, 4]], tupe, 这个21824为个5个尺度所有的anchor总数，因此后面的4代表anchor的偏移量
        # print(len(pos_inds_list))
        # print(pos_inds_list.shape)
        # print(pos_inds_list)
        # 每个list的长度为8对应的是bachsize
        # print(len(anchor_list))
        # print(anchor_list[0].shape)
        # print(anchor_list[1].shape)
    ...
    def _get_target_single(self,
        ...
        # num_gts, gt_inds, max_overlaps, labels
        sampling_result = self.sampler.sample(assign_result, anchors,
                                              gt_bboxes)
        # print(sampling_result)
        # print(sampling_result.pos_assigned_gt_inds.shape)
        # print(sampling_result.pos_inds.shape)
        # print("***")
        # neg_bboxes, ned_inds, num_gts
        # pos_assigned_gt_inds, pos_bboxes, pos_inds, pos_is_gt
        ...
        # print("**")
        # todo bbox_weights=[anchors_num,4]
        # todo label_weighs=[anchors_num]
        ...
        # print(pos_inds)
        # pos_overlaps = assign_result.max_overlaps[pos_inds]
        # print(assign_result.max_overlaps[pos_inds])
        # print(pos_overlaps)

        # 根据将不同的正样本分配给不同的gt，利用gt_bboxes的面积对已经分配好的真值进行面积分配，
        # 其中所有正样本的已经根据pos_assigned_gt_inds, 分配到不同的gt上，之后根据真值的面积来做加权
        # 采用真值的面积除以标准面积，得到归一化的面积，然后利用指数函数进行赋予权重
        # 为目标构建label以及label的权重，bbox以及bbox的权重。
        area = torch.sqrt((gt_bboxes[:, 2] - gt_bboxes[:, 0]) * (gt_bboxes[:, 3] - gt_bboxes[:, 1]))
        area_weight = torch.exp(-area / 512) + 1
        # todo 3倍权重的差距网络
        area_weight1 = 3 * torch.exp(-area / 512)
        # todo 2倍权重差距网络
        area_weight2 = 2 * torch.exp(-area / 512)
        # todo area=area_weight=[gt_nums], area 值=[0~1024], area_weight 值=[0~2]
        # print(area.shape)
        # print(area)
        # print(area_weight1.shape)
        # print(area_weight1)
        # print("**")
        # print(sampling_result.pos_assigned_gt_inds)
        # todo wei是根据真值面积分配的权重。todo wei需要转置一下
        # todo 其中wei是为了分类的label设置的权重，wei1是为了回归的框的设置的权重
        wei = area_weight[sampling_result.pos_assigned_gt_inds]
        wei1 = wei.unsqueeze(-1)
        # todo wei=[anchor_num] 值为上述area_weight的值，根据每个正样本划分为对应的gt。并且把gt所属于的面积权重划分给对应的anchor权重
        # todo wei1=[anchor_num, 1] 有点类似于转置
        # print("&&&")
        # print(wei)
        # print(wei.shape)
        # print(wei1)
        # print(wei1.shape)
        # exit()
        # Todo 修改对应的权重  bbox_weights[pos_inds, :] = 1.0  label_weights[pos_inds]
        if len(pos_inds) > 0:
            pos_bbox_targets = self.bbox_coder.encode(
                sampling_result.pos_bboxes, sampling_result.pos_gt_bboxes)
            bbox_targets[pos_inds, :] = pos_bbox_targets
            # todo 原始 bbox权重全部设置为1
            # bbox_weights[pos_inds, :] = 1.0
            # todo area 的bbox权重设置为对应的面积加权
            ...
            # print("***")
            # print(self.background_label) #背景定义为10
            # print(labels)
            # print(pos_inds)
    ...
    def loss_reg_single(self, anchors, cls_score, bbox_pred, centerness, labels,
        ...
        if len(pos_inds) > 0:
            ...
            pos_anchors = torch.zeros((1, 4), dtype=torch.int64).cuda()
        ...
        # print("&&&")
        # exit()
        return centerness_targets.sum(), pdbp, pbd, pos_overlaps_list, ious, pos_centerness, centerness_targets, pos_anchors
    ...
    def loss_reg(self,
        ...
        bbox_avg_factor, pos_decode_bbox_pred, pos_bbox_pred, pos_overlaps_lists, ious, pos_centerness, centernesses_targets, pos_anchors = multi_apply(
        ...
        bbox_avg_factor = sum(bbox_avg_factor)
        bbox_avg_factor = reduce_mean(bbox_avg_factor).item()
        return ious, pos_decode_bbox_pred, pos_bbox_pred, pos_overlaps_lists, pos_centerness, centernesses_targets, pos_anchors
