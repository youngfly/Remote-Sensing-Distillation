...
class ATSSHead2(AnchorHead):
    ...
    def __init__(self,
        ...
        super(ATSSHead2, self).__init__(num_classes, in_channels, **kwargs)
    ...
    def loss_stu(self,
        ...
        pos_inds_flatten_old = ((labels_old >= 0)
                            &
                            (labels_old < self.num_classes)).nonzero().reshape(-1)
        ...
        # todo 由于重新分配的样本
        # todo reassign_labels, reassign_label_weight, reassign_bbox_weights
        # todo labels,          labesl_weight,       ,
        # todo 得到所需要的正样本的分类分数以及回归分数,从这里开始的所有的bath的样本的全部被拼接在一起！！！！！！！
        cls_scores = torch.cat(cls_scores, 0).view(-1, cls_scores[0].size(-1))
        bbox_preds = torch.cat(bbox_preds, 0).view(-1, bbox_preds[0].size(-1))

        # todo 通过重新分配之后, 得到新分配的labels，以及label的权重， 用此可以筛选出来所有正样本。
        ...
        # todo 最后的一个类为背景, 将所有的类别的标签进行展开
        # todo 这个就是最终拿到的正样本的序号！！！！！！！
        ...
        pos_labels = labels[pos_inds_flatten]
        pos_cls = cls_scores[pos_inds_flatten]
        pos_index = batch_index_all[pos_inds_flatten]
        pos_gt = batch_gt_all[pos_inds_flatten]
        ...
        if num_pos:
            ...
            pos_bbox_target = self.bbox_coder.decode(
                flatten_anchors[pos_inds_flatten],
                bboxes_target[pos_inds_flatten]
            )
            ...
            pos_cls_loss, pos_reg_loss = self.get_cr_loss(pos_cls, pos_bbox_pred, pos_labels,
                                                          pos_bbox_target, pos_inds_flatten, flatten_anchors)
        else:
        ...
        return pos_cls, pos_bbox_pred, pos_labels, pos_index, pos_gt, pos_inds_flatten, \
               pos_cls_loss, pos_reg_loss, pos_inds_flatten_old, pos_bbox_target
    ...
    def loss_tea(self,
        ...
        # todo 获取训练的目标 由于在设置中 pos的iou阈值设置为0.1，采用maxiou先挑选一大批的anchor，但是pos_anchor基本位于gt附近
        cls_reg_targets = self.get_paa_targets(
            anchor_list,
            valid_flag_list,
            gt_bboxes,
        ...
        if len(flatten_anchors):
            ...
            pos_cls_loss, pos_reg_loss = self.get_cr_loss(pos_cls_scores, pos_bbox_pred, pos_labels,
                                                          pos_bboxes_target, pos_inds_flatten, flatten_anchors)
        else:
            pos_bbox_pred = torch.zeros([1, 4], dtype=torch.int64)
            pos_cls_loss = torch.tensor([0.0]).cuda()
            pos_reg_loss = torch.tensor([0.0]).cuda()
        ...
        return pos_cls_scores, pos_bbox_pred, pos_cls_loss, pos_reg_loss
    ...
    def get_cr_loss(self, cls_score, bbox_pred, label,
                     bbox_target, pos_inds, flatten_anchors):
        if not len(pos_inds):
            return cls_score.new([]),
        # todo 获取所有的正样本的分数
